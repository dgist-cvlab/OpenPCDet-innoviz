...
class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None):
        ...
        self.configs_for_sweep = augmentor_configs.get('CONF_FOR_SWEEP', None)
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        points, gt_boxes = augmentor_utils.scale_pre_object(
            ...
            scale_perturb=config['SCALE_UNIFORM_NOISE_DETAIL'],
        )
    ...
    def random_world_translation(self, data_dict=None, config=None):
        ...
        noise_translate_off = config['NOISE_TRANSLATE_OFFSET']
        assert len(noise_translate_std) == 3
        noise_translate = np.array([
            np.random.normal(0, noise_translate_std[0], 1) + noise_translate_off[0],
            np.random.normal(0, noise_translate_std[1], 1) + noise_translate_off[1],
            np.random.normal(0, noise_translate_std[2], 1) + noise_translate_off[2],
        ], dtype=np.float32).T
    ...
    def shadow_random_drop(self, data_dict=None, config=None):
        ...
        if self.configs_for_sweep is not None:
            factor = self.configs_for_sweep.RATIO_FACTOR
        else:
    ...
    def shadow_jitter(self, data_dict=None, config=None): # TODO: not completed function
        if data_dict is None:
            return partial(self.shadow_jitter, config=config)
        gt_boxes, points = data_dict['gt_boxes_aug2'], data_dict['points_aug2']

        if self.configs_for_sweep is not None:
            xfactor = float(self.configs_for_sweep.JITTER_FACTOR)
            factor = [xfactor, xfactor/4, xfactor/4]
        else:
            factor = config['JITTER_FACTOR']

        noise = np.random.normal(0, factor, (points.shape[0], 3))
        points[:,0:3] += noise 
