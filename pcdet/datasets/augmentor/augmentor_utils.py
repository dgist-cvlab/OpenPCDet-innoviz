...
def scale_pre_object(gt_boxes, points, gt_names, scale_perturb, num_try=5):
    """
    ...
    # scale_perturb 
    # scale_noises = [for i in range()]

    # boxes wise scale ratio per class
    perturb_per_class = np.asarray(list(map(lambda x: scale_perturb[x], gt_names)))
    scale_noises = np.asarray([np.random.uniform(b[0], b[1], size=[1, num_try]) for b in perturb_per_class]).reshape(num_boxes, -1)
    ...
    for k in range(num_boxes):
        ...
        obj_center, lwh, ry = gt_boxes[k, 0:3], gt_boxes[k, 3:6], gt_boxes[k, 6:7]
