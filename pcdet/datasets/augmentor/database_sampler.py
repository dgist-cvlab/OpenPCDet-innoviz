...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene_with_aug(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None, sampled_mean_floor=None,
                                            random_dict=None, ):
        gt_boxes_mask = data_dict['gt_boxes_mask']
        gt_boxes = data_dict['gt_boxes'][gt_boxes_mask]
        gt_names = data_dict['gt_names'][gt_boxes_mask]
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            # obj_points[:, :3] += info['box3d_lidar'][:3].astype(np.float32) # move box center point to real world coord

            ### AUGMENTATION ###
            # ori_range = np.linalg.norm(info['box3d_lidar'][:3])
            # aug_range = abs(np.random.normal(0, 0.3)) + 1.0
            aug_range = info['aug_range']
            aug_angle = info['aug_angle']
            # aug_angle = 0.0
            # # if np.random.random() < 0.8:
            # if random_dict['angle_binary_random'][idx] < 0.2:
            #     aug_angle = random_dict['angle_random1'][idx]
            # else:
            #     aug_angle = random_dict['angle_random2'][idx]
            
            # drop points as range
            select_num = np.long(max(np.long(obj_points.shape[0]) * (1 / aug_range*aug_range), 5))
            selected = np.random.randint(0, obj_points.shape[0], select_num)
            obj_points = obj_points[selected] 

            # xyz = info['box3d_lidar'][:3]
            ac_ = np.cos(aug_angle)
            as_ = np.sin(aug_angle)
            rot_mat = np.array([[ac_, -as_, 0], [as_, ac_, 0], [0, 0, 1]])
            # np.matmul(obj_points[:, :3], rot_mat, out=obj_points[:, :3])
            # xyz = np.matmul(xyz, rot_mat)
            # xyz[:2] = xyz[:2] * aug_range
            # angle = info['box3d_lidar'][6:7]
            # angle = (angle - aug_angle) % (2 * np.pi)

            # if sampled_mean_floor is not None:
            #     box_floor = np.mean(info['box3d_lidar'][2] - info['box3d_lidar'][5] / 2)
            #     gt_est_floor = np.random.uniform(sampled_mean_floor[0], sampled_mean_floor[1]) # mean ~ median
            #     z_offset = gt_est_floor - box_floor
            #     xyz[2] += z_offset
            # augmented_box = np.concatenate([xyz, info['box3d_lidar'][3:6], angle], axis=0)

            # move box center point to real world coord
            obj_points[:, :3] = np.matmul(obj_points[:, :3], rot_mat)
            obj_points[:, :3] += sampled_gt_boxes[idx][:3].astype(np.float32)  

            # Give augmented box
            # sampled_gt_boxes[idx] = info['box3d_lidar'].astype(np.float32)
            ### AUGMENTATION ###
    ...
    def __call_old__(self, data_dict):
        """
        Args:
            data_dict:
                gt_boxes: (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...]
        ...
        gt_floors = gt_boxes[:, 2:3] - gt_boxes[:, 5:6] / 2
        sampled_mean_floor = [np.median(gt_floors), np.mean(gt_floors)]
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            data_dict = self.add_sampled_boxes_to_scene_with_aug(
                data_dict, sampled_gt_boxes, total_valid_sampled_dict, sampled_mv_height, sampled_gt_boxes2d, sampled_mean_floor=sampled_mean_floor
            )
    ...
    def __call__(self, data_dict):
        ...
        gt_floors = gt_boxes[:, 2:3] - gt_boxes[:, 5:6] / 2
        if len(gt_floors) == 0:
            sampled_mean_floor = None
        else:
            sampled_mean_floor = [np.median(gt_floors), np.mean(gt_floors)]

        ## set random ##
        range_random = abs(np.random.normal(0, 0.3, 200)) + 1.0
        angle_binary_random = np.random.random()
        angle_random1 = np.random.normal(0, 0.15)
        angle_random2 = np.random.uniform(-np.pi / 4, np.pi / 4)
        random_dict = {
            'range_random': range_random,
            'angle_binary_random': angle_binary_random,
            'angle_random1': angle_random1,
            'angle_random2': angle_random2,
        }
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                for idx, info in enumerate(sampled_dict):
                    ...
                    if np.random.random() < 0.7:
                        aug_angle = np.random.normal(0, 0.15)
                    else:
                        aug_angle = np.random.uniform(-np.pi / 4, np.pi / 4)
                    ...
                    if sampled_mean_floor is not None:
                        ...
                        xyz[2] += z_offset
                    augmented_box = np.concatenate([xyz, info['box3d_lidar'][3:6], angle], axis=0)
                    info['aug_range'] = aug_range
                    info['aug_angle'] = aug_angle
                    ### AUGMENTATION ###
                    aug_boxes.append(augmented_box)
                sampled_boxes = np.stack(aug_boxes, axis=0).astype(np.float32)
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            data_dict = self.add_sampled_boxes_to_scene_with_aug(
                data_dict, sampled_gt_boxes, total_valid_sampled_dict, sampled_mv_height, sampled_gt_boxes2d, sampled_mean_floor=sampled_mean_floor, random_dict=random_dict,
            )
