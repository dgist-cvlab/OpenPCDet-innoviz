...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.class_names_map = self.dataset_cfg.get('CLASS_NAME_MAP', None)
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                for kk in range(0, len(infos)):
                    if self.class_names_map is not None and infos[kk]['annos']['name'].size != 0:
                        infos[kk]['annos']['name'] = np.vectorize(self.class_names_map.get)(infos[kk]['annos']['name'])
        ...
        elif self.dataset_cfg.SAMPLED_INTERVAL[mode] < 0: # only use whic semantic label exists
            self.infos = self.sample_data_with_sem_label(-self.dataset_cfg.SAMPLED_INTERVAL[mode])
            self.logger.info('Total sampled samples for Waymo dataset (w/ sem_label): %d' % len(self.infos))
    ...
    def sample_data_with_sem_label(self, skip_num=1):
        sampled_waymo_infos = []
        info_cnt = 0
        for k in range(0, len(self.infos)):
            # if self.infos[k]['point_cloud']['sem_label'] is not None:
            #     sampled_waymo_infos.append(self.infos[k])
            sequence_name = self.infos[k]['point_cloud']['lidar_sequence']
            sample_idx = self.infos[k]['point_cloud']['sample_idx']
            sem_label_file :Path = self.data_path / sequence_name / 'sem_label' / ('%04d.npy' % sample_idx)
            if sem_label_file.exists():
                info_cnt += 1
                if info_cnt % skip_num == 0:
                    sampled_waymo_infos.append(self.infos[k])
        return sampled_waymo_infos           
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1, update_info_only=False, do_semantic_label=False):
        ...
        process_single_sequence = partial(
            ...
            save_path=save_path, sampled_interval=sampled_interval, has_label=has_label, update_info_only=update_info_only, do_semantic_label=do_semantic_label
        )
    ...
    def __getitem__(self, index):
        ...
        if self.only2d:
            gt_boxes_yesz = copy.deepcopy(data_dict['gt_boxes'])
            data_dict['gt_boxes'][:, 2] = 1.0
            data_dict['gt_boxes'][:, 5] = 5.9
            data_dict['gt_boxes_aug2'][:, 2] = 1.0
            data_dict['gt_boxes_aug2'][:, 5] = 5.9
            data_dict['gt_boxes_yesz'] = gt_boxes_yesz
        data_dict['correct_gt'] = 1.0
        return data_dict
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        for k in tqdm(range(0, len(infos), sampled_interval)):
            ...
            if True:
                gt_boxes = annos['gt_boxes_lidar'][:, 0:7] # use no speed
            else:
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    waymo_infos_val = dataset.get_infos(
        ...
        sampled_interval=1, update_info_only=update_info_only, do_semantic_label=do_semantic_label,
    )
...
if __name__ == '__main__':
    ...
    if args.func == 'create_waymo_infos':
        ...
        create_waymo_infos(
            ...
            do_semantic_label=args.do_semantic_label,
        )
