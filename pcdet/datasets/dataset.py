...
# from .detr_utils import getitem
...
class DatasetTemplate(torch_data.Dataset):
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['gt_boxes', 'gt_boxes_aug2', 'gt_boxes_yesz']:
    ...
    def fit_to_3detr(self, batch_dict):
        # for infer
        pc = copy.deepcopy(batch_dict["points"])
        bbox = copy.deepcopy(batch_dict["gt_boxes"])
        bbox[:, -1] -= 1
        if bbox[:,-1].min() < 0:
            print("Error: bbox[:,-1].min() < 0")
        detr_dict = getitem(pc, bbox)
