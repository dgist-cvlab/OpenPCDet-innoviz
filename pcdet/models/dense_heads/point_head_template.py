...
class PointHeadTemplate(nn.Module):
    ...
    def get_cls_layer_loss_with_mask(self, mask=None, tb_dict=None):
        point_cls_labels = self.forward_ret_dict['point_cls_labels'].view(-1)
        point_cls_preds = self.forward_ret_dict['point_cls_preds'].view(-1, self.num_class)
        point_batch_ids = self.forward_ret_dict['point_batch_ids'].view(-1)
        
        # torch.index_select(torch.Tensor(mask).to('cuda'), 0, point_batch_ids.long())
        mask_index = torch.index_select(mask, 0, point_batch_ids.long()).bool()
        point_cls_labels[~mask_index] = -1
