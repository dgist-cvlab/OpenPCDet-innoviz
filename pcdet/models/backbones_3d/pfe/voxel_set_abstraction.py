...
def sample_points_with_roi2(rois, points, sample_radius_with_roi, num_max_points_of_part=200000):
    """
    Args:
        rois: (M, 7 + C)
        points: (N, 3)
    ...
    if points.shape[0] < num_max_points_of_part:
        ...
        point_mask = min_dis < roi_max_dim + sample_radius_with_roi
        if point_mask.sum() == 0:
            help_dist = min_dis - (roi_max_dim + sample_radius_with_roi) + 0.1
            point_mask = min_dis < roi_max_dim + sample_radius_with_roi + help_dist.min()
    else:
        start_idx = 0
        point_mask_list = []
        while start_idx < points.shape[0]:
            ...
            roi_max_dim = (rois[min_dis_roi_idx, 3:6] / 2).norm(dim=-1)
            cur_point_mask = min_dis < roi_max_dim + sample_radius_with_roi
            if cur_point_mask.sum() == 0:
                help_dist = min_dis - (roi_max_dim + sample_radius_with_roi) + 0.1
                cur_point_mask = min_dis < roi_max_dim + sample_radius_with_roi + help_dist.min()
            point_mask_list.append(cur_point_mask)
            start_idx += num_max_points_of_part
        point_mask = torch.cat(point_mask_list, dim=0)
    ...
    def aggregate_keypoint_features_from_one_source(
        ...
        if filter_neighbors_with_roi:
            ...
            for bs_idx in range(batch_size):
                ...
                _, valid_mask = sample_points_with_roi2(
