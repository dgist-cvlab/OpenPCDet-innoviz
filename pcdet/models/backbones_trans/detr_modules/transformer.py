...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_args, num_layers,
                 norm=None, weight_init_name="xavier_uniform"):
        super().__init__()
        # self.layers = get_clones(encoder_layer, num_layers)
        # self.layers = nn.ModuleList([TransformerEncoderLayer(encoder_args) for i in range(num_layers)])
        self.add_module(
            'layers',
            nn.ModuleList([TransformerEncoderLayer(**encoder_args) for i in range(num_layers)])
        )
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=128,
        ...
        if self.use_ffn:
            ...
            # inplace torch 1.10 error. refer https://discuss.pytorch.org/t/inplace-errors-with-dropout-layers-with-pytorch-1-9-but-not-with-pytorch-1-10/137544/10
